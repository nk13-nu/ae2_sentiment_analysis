...
@app.get("/sentiment/{text}", summary="Input a product or service review and get a sentiment score across six emotions", 
         description="This endpoint takes a product review as text and returns six different scores across six emotions.", response_model=APIResponse)
def analyse_sentiment(text:str):
    prediction = SentimentAnalyser(text=text).get_sentiment()
    sentiment_warehouse.store(prediction)
    return prediction

@app.get("/analyse/allSentiments", summary="Retrieve all sentiments for each piece of text in this session.")
def all_sentiments():
    return sentiment_warehouse.get_all_sentiments()

@app.get("/analyse/averageSentiment", summary="Retrieve the average sentiment per emotion across all current product reviews.")
